...
def test_skyflux():
    ...
    """
    skyflux = cg.SkyFlux()
    skyflux.run_skycalc('./test.fits')
    os.remove('./test.fits')
    os.remove('./skycalc_params.txt')
    os.remove('./almanac_params.txt')
    fig, ax = skyflux.plot_skyflux()

def test_count_rates_ground():
    """
    Parameters
    ----------
    ...
    # Initiate a SkyFlux
    skyflux = cg.SkyFlux()
    skyflux.wmin = 0.3 * 1000
    skyflux.wmax = 2.6 * 1000
    skyflux.run_skycalc('./test.fits')
    # delete the files
    os.remove('./test.fits')
    os.remove('./skycalc_params.txt')
    ...
    lam, dlam, A, q, Cratio, cp, csp, cz, cez, cD, cR, cth, cc, DtSNR = \
        cg.count_rates(Ahr, lamhr, solhr, alpha, Phi, Rp, Teff, Rs, r, d, Nez,\
                       lammax=1.6, THERMAL = True, GROUND = "SKYFLUX", vod=False, skyflux=skyflux)


    # Run wrapper
    out = cg.count_rates_wrapper(Ahr, lamhr, solhr, cg.Telescope(), cg.Planet(),
                                 cg.Star(), GROUND = "SKYFLUX", skyflux=skyflux)
...
def test_transits():
    ...
    # Initiate a SkyFlux
    skyflux = cg.SkyFlux()
    skyflux.wmin = telescope.lammin * 1000 + 100 # have to overshoot to avoid edge effects
    skyflux.wmax = telescope.lammax * 1000 + 100 # have to overshoot to avoid edge effects
    skyflux.run_skycalc('./test.fits')
    # delete the files
    os.remove('./test.fits')
    os.remove('./skycalc_params.txt')
